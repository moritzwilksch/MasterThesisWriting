...
In this section, we demonstrate an example use-case which utilizes the proposed sentiment analysis model. The example we choose is purely descriptive but shows how document-level market sentiment obtained from the proposed model can be aggregated as a time series to study a phenomenon.
As an example, we study the GameStop short squeeze in early 2021. Inspired by an investment strategy proposed on YouTube, a group of retail investors collectively organized on various social media platforms to buy stock in the video game retailer GameStop, increasing its value by 30-fold within three months. The rapid increase in GameStop's stock price forced Melvin Capital, a hedge fund with a significant short position in GameStop, to cover their position resulting in losses big enough to force them to seize operations in 2022. Its public nature and the online discussions that caused this short squeeze make it a prime example for assessing the social media sentiment regarding the GameStop stock during this time period.\newline
