\begin{figure}[!ht]
	...
	\node[draw=grey, right=0.25cm of 1,align=left,text width=9cm,minimum height=1.4cm](1text) {Previous literature often uses generic models for SA on finance-related social media posts because there are few functional artifacts for this domain.};
	
	\node[draw=grey,below=0.25cm of 1.south,fill=lightgrey,text width=3cm,align=center,minimum height=1.1cm](2) {\textbf{Suggestion}};
	\node[draw=grey,below=0.25cm of 1text,align=left,text width=9cm,minimum height=1.1cm](2text) {Develop a domain-specific model that can classify finance-related SM posts as bullish, bearish, or neutral.};
	...
	\node[draw=grey,below=0.25cm of 3text,align=left,text width=9cm,minimum height=1cm](4text) {Compare the achieved performance against the accuracy of other models from the domain of finance or SM.};
	...
	\end{tikzpicture}
	\caption{Design Science Research process adapted from \protect\shortciteA{kuechler2012dsrprocess}}
