...
\begin{figure}[!ht]
	...
	\node[draw=grey,below=0.25cm of 1text,align=left,text width=9cm,minimum height=1.1cm](2text) {Develop a domain-specific model that can classify financial Tweets as bullish, bearish, or neutral.};
	
	\node[draw=grey,below=0.25cm of 2.south,fill=lightgrey,text width=3cm,align=center,minimum height=1.5cm](3) {\textbf{Development}};
	\node[draw=grey,below=0.25cm of 2text,align=left,text width=9cm,minimum height=1.5cm](3text) {Collect and label data, clean and preprocess text, build and compare multiple ML \& DL models, deploy the best models as a python library.};
	...
	\node[draw=grey,below=0.25cm of 4.south,fill=lightgrey,text width=3cm,align=center,minimum height=0.6cm](5) {\textbf{Conclusion}};
	\node[draw=grey,below=0.25cm of 4text,align=left,text width=9cm,minimum height=0.6cm](5text) {Recommend optimal SA model for different use cases.};	
	...
	\end{tikzpicture}
	\caption{Design Science Research process adapted from \protect\shortciteA{gregor2013dsr} \textcolor{blue}{applied to the topic of this work}}
	\label{dsr-process}
\end{figure}
